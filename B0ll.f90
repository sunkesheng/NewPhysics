Real(dp) :: AmpSquared,AmpSquared2,AmpSquared11,AmpSquared22, AmpSquared_SM, AmpSquared2_SM, &
              & width_SM, width, width11, width22
...
Else if (i1.eq.2) Then ! B0s -> e mu
  ...
  gt3 = 1
  gt4 = 2
Else if (i1.eq.3) Then ! B0d -> e tau
  MassB0 = MassB0d
  MassB02 = MassB0d**2
  fB0 = fBd
  GammaB0 = (hbar)/(TauB0d)
  ...
  gt3 = 1
  gt4 = 3
Else if (i1.eq.4) Then ! B0s -> e tau
  MassB0 = MassB0s
  MassB02 = MassB0s**2 
  fB0 = fBs
  GammaB0 = (hbar)/(TauB0s)
  ...
  gt3 = 1
  gt4 = 3
Else if (i1.eq.5) Then ! B0d -> mu tau
  MassB0 = MassB0d
  MassB02 = MassB0d**2
  fB0 = fBd
  GammaB0 = (hbar)/(TauB0d)
  ...
  gt3 = 2
  gt4 = 3
Else if (i1.eq.6) Then ! B0s -> mu tau
  MassB0 = MassB0s
  MassB02 = MassB0s**2 
  fB0 = fBs
  GammaB0 = (hbar)/(TauB0s)
...
AmpSquared11 = 2 * abs(FS)**2 * (MassB02 - (mf_l(gt3)+mf_l(gt4))**2) &
     & + 2 *abs(FP)**2 * (MassB02 - (mf_l(gt3)-mf_l(gt4))**2) &
     & + 2 *abs(FV)**2 * (MassB02*(mf_l(gt4)-mf_l(gt3))**2    &
             & - (mf_l2(gt4)-mf_l2(gt3))**2) &
     & + 2 *abs(FA)**2 * (MassB02*(mf_l(gt4)+mf_l(gt3))**2 -  &
...
width11 = oo16pi * AmpSquared11 / MassB0 * &
    & sqrt(1-((mf_l(gt4)+mf_l(gt3))/MassB0)**2) &
    & * sqrt(1-((mf_l(gt4)-mf_l(gt3))/MassB0)**2)*(Alpha/Alpha_160)**4

! Bsd->lp+ l-

CS(1) = OddllSRR(gt1,gt2,gt4,gt3)
CS(2) = OddllSRL(gt1,gt2,gt4,gt3)
CS(3) = OddllSLL(gt1,gt2,gt4,gt3)
CS(4) = OddllSLR(gt1,gt2,gt4,gt3)

CV(1) = OddllVLL(gt1,gt2,gt4,gt3)
CV(2) = OddllVLR(gt1,gt2,gt4,gt3)
CV(3) = OddllVRR(gt1,gt2,gt4,gt3)
CV(4) = OddllVRL(gt1,gt2,gt4,gt3)
...
AmpSquared22 = 2 * abs(FS)**2 * (MassB02 - (mf_l(gt4)+mf_l(gt3))**2) &
     & + 2 *abs(FP)**2 * (MassB02 - (mf_l(gt4)-mf_l(gt3))**2) &
     & + 2 *abs(FV)**2 * (MassB02*(mf_l(gt3)-mf_l(gt4))**2    &
             & - (mf_l2(gt3)-mf_l2(gt4))**2) &
     & + 2 *abs(FA)**2 * (MassB02*(mf_l(gt3)+mf_l(gt4))**2 -  &
             & (mf_l2(gt3)-mf_l2(gt4))**2) &
     & + 4 *REAL(FS*conjg(FV)) *(mf_l(gt4)-mf_l(gt3)) *(MassB02 &
             & + (mf_l(gt4)+mf_l(gt3))**2) &
     & + 4 *REAL(FP*conjg(FA)) *(mf_l(gt4)+mf_l(gt3)) *(MassB02 &
             & - (mf_l(gt4)-mf_l(gt3))**2) 

width22 = oo16pi * AmpSquared22 / MassB0 * &
    & sqrt(1-((mf_l(gt3)+mf_l(gt4))/MassB0)**2) &
    & * sqrt(1-((mf_l(gt3)-mf_l(gt4))/MassB0)**2)*(Alpha/Alpha_160)**4

If (i1.Eq.1) Then
  BrB0dEMu= (width11+width22) / GammaB0
Else If (i1.Eq.2) Then
  BrB0sEMu= (width11+width22) / GammaB0
Else If (i1.Eq.3) Then
  BrB0dETau= (width11+width22) / GammaB0
Else If (i1.Eq.4) Then
  BrB0sETau= (width11+width22) / GammaB0
Else If (i1.Eq.5) Then
  BrB0dMuTau= (width11+width22) / GammaB0
Else If (i1.Eq.6) Then
  BrB0sMuTau= (width11+width22) / GammaB0
End If
